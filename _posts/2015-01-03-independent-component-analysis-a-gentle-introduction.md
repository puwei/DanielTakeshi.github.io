...
the microphones provide us with voice measurements $latex x\_1^{(i)}$ and $latex x\_2^{(i)}$, such
as amplitudes.
...
We describe a simple stochastic gradient descent algorithm to learn the parameter $latex A^{-1}$ of
...
There are whole books written on how to decide when to stop iterating, so I won't get into
